...
language, just as much as any other out there. The nice thing about computers is that they can tell you if
they could or could not understand you (most of the time), and often extremely fast.
...
read code is a great mental exercise for anybody, and one that can be very rewarding when you least
expect it. This resource will be based around a personal project I created specifically with the
...
this experience leaves you with an understanding of any code you happen to lay eyes on,
...
thing I can compare this process to (perhaps coincidentally, and somewhat keeping in theme with the site you will
learn about in intimate detail) is learning music. This approach will be akin to teaching you an instrument,
...
when you save information, but you will start to see this in the following examples. Here is how you would do the
same thing in TypeScript and Javascript.
...
You will notice `//` before some of the lines. These designate a comment is being placed, and are
...
64-bit). It is specified by the language that `int` cannot be below 2 bytes (16-bits).

Do not feel like you have to memorize this! This information is easily looked up, and once you understand
the reasoning behind these things, you realize the theme is more important than being able to memorize them.
...
computed, this would require an additional bit to represent as a value. Powers of 2 explain why you
always see things related to computers, such as hard drives and other hardware, with amounts like 1TB,
512GB, 256MB, 128KB, or 64B. In all of those amounts the B is Bytes, the prefix is from the metric system.
Here are the basics:
...
Bits really are nothing more than switches for on and off states. You can see that their values and
positions offer ways to generate as many numbers as we want, we can always just add more zeros. As a reminder,
the values start at the right. Bytes and bits also can also be represented in hexadecimal, which is base
...
systems work the exact same way. Hexadecimal is nice because you can write much bigger numbers using
...
is not at first obvious that this is the case though. You may be familiar with a number like 0.5, which
...
This explains why you may run into questions from others about why something like the following is given as
an output:
...
With TypeScript, `number` actually takes on each of these cases (and others), so you do no have to worry as much
about the details. This has been a common theme in many languages, limiting the burden for those
...
work. Learning the specifics of these topics is something that can be learned overtime as you are making
...
Maybe some reading this will not think the expression above (also referred to as a variable assignment), is
all that impressive. On some level, they are right. Assigning a variable with an equals sign is not
...
reassign a variable in TypeScript/JavaScript you must use the keyword `let`:
...
This was pointed out due to this possibly being something others are not used to, even coming from
